...
| Agy | Nudge | No tracked registration. | Agy 1.1.8's installed lifecycle-hook contract exposes PreToolUse, PostToolUse, PreInvocation, PostInvocation, and Stop, but no SessionStart event, so this native axis is not applicable and the tracked `AGENTS.md` session-start instruction remains the enforcement. |
